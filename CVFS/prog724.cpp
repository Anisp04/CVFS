...
{
    char ProcessName[50];
    PFILETABLE UFDT[MAXINODEOPENEDFiles];
};
...
void DisplayHelp()
{
    ...
    printf("Unlink : It is used to Unlink the regualar file\n");
    printf("stat : It is used display statistiacl information file\n");
    printf("ls : It is used to list out all file from directory\n");
    printf("write : It is used to write the data into the file\n");
    printf("It is use read data from the file\n");
            ...
            )
{
    ...
    {
        printf("Description : This command is used to statistical information about file\n");

        printf("Usage : stat file_name\n");
        printf("File_name : The name of file whose information you want \n");
    ...
    else if(strcmp(name,"read") == 0)
    {
        ...
        printf("FD = It is value returned by creat system call\n");
    ...
    for(i = 0; i < MAXINODE;i++)
    {
        if(uareaobj.UFDT[i] != NULL)
        {
            if(strcmp(uareaobj.UFDT[i] -> ptrinode->FileName,name)== 0)
            {
                ...
                uareaobj.UFDT[i]->ptrinode->InodeNumber = i;
                ...
                //set NULL UFDT member
                uareaobj.UFDT[i] = NULL;

                //Increament of freeinode count;
                superobj.FreeInodes++;
...
//  Description :       It is used to display a information about all files in the directory
//  Input :             Nothing
//  Output :            Nothing
//  Author :            Vinay Bharat Khot
//  Date :              15/08/2025
...
//  Description :       It is used to display a information about given file
//  Input :             Nothing
//  Output :            Nothing
//  Author :            Vinay Bharat Khot
//  Date :              15/08/2025
                ...
                )
{
    ...
    while(temp != NULL)
    {
        ...
        {
            printf("--------------------------Statistiacal Information of file---------------------\n");
            printf("File Name : %s\n",temp->FileName);
            printf("File size on disk : %d\n",temp->FileSize);
            printf("Actual File Size : %d\n",temp->ActualFileSize);
            printf("Link Count : %d\n",temp -> LinkCount);
            printf("File Permissioin : ");
            if(temp -> Permission == WRITE)
            {
                printf("Write\n");

            }
            else if(temp -> Permission == READ)
            {
                printf("Read\n");
...
//  Description :       It is used to write a data in file
//  Input :             file_descriptor, Address of buffer, size of data that we want to the write
//  Output :            Number of bytes successfully written in file
//  Author :            Vinay Bharat Khot
//  Date :              15/08/2025
//
///////////////////////////////////////////////////////////////////////
...
int write_file(
                ...
                ,int size   //Size of data we want
                )
{
    unsigned long int offset = 0;
    printf("File Discriptor is : %d\n",fd);
    printf("Data that we want to write : %s\n",data);
    printf("Number of bytes that you want write : %d\n",size);

    //Invalid value of fd
    if(fd < 0 || fd> MAXINODEOPENEDFiles)
    {
        return ERR_INVALID_PARAMETER;
    }
    ...
    //if there is no permission to write th data into the file
    if(uareaobj.UFDT[fd]->ptrinode->Permission < WRITE)
    {
        return ERR_PERMISSION_DENIED;
    }

    //Unable to write as there no sufficient space
    ...
    //Update the actual size of file after writig new data
...
//  Description :       It is used to read contain from file
//  Input :             file_descriptor, Address of Emptybuffer, size of data that we want to the read
//  Output :            Number of bytes successfully read in file
//  Author :            Vinay Bharat Khot
//  Date :              15/08/2025
//
///////////////////////////////////////////////////////////////////////
                ...
                )
{
    ...
    //Unable to read as there no sufficient space
    ...
    while(1)
    {
        ...
        else if(iCount == 2)
        {
            ...
            else if(strcmp(Command[0],"unlink") == 0)
            {
                ...
                {
                    printf("File Unlink Operation is succesfully performed\n");
                }
                else if(iRet == ERR_FILE_NOT_EXIST)
                {
                    printf("Error : Unable to do unlink\n");
                }
            ...
            else if(strcmp(Command[0],"stat") == 0)
            {
                ...
                {
                    printf("Error : Unable to diaplay statistics as file not present\n");
                }
                else if(iRet == ERR_INVALID_PARAMETER)
                {
                    printf("Error : Invalid parameters for the function\n");
            ...
            {
                printf("Please enter the data that you want to write\n");
                scanf("%[^'\n']s",InputBuffer);
                ...
                {
                    printf("Error : Unable to write there is no permission\n");
                }
                else if(iRet == ERR_INVALID_PARAMETER)
                {
                    printf("Error : Invalid parameters for the function\n");
                ...
                {
                    printf("%d bytes gtes succefully write into a file\n",iRet);
        ...
        else if(iCount == 3)
        {
            ...
            else if(strcmp(Command[0],"read") == 0)
            {
                ...
                {
                    printf("Error : Unable to read there is no permission\n");
                }
                else if(iRet == ERR_INVALID_PARAMETER)
                {
                    printf("Error : Invalid parameters for the function\n");
                ...
                {
                    printf("read operation succefull\n");
                    printf("Data from file is :%s\n",EmptyBuffer);
